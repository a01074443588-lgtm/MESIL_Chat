...
function objectMessage(value: unknown): string | null {
  ...
  const candidate = value as { message?: unknown; msg?: unknown };
  for (const message of [candidate.message, candidate.msg]) {
...
export function apiErrorMessage(payload: unknown, status: number): string {
  ...
  if (Array.isArray(detail)) {
    const messages = [
      ...new Set(
        detail
          .map((item) => objectMessage(item))
          .filter((message): message is string => Boolean(message)),
      ),
    ];
    if (messages.length) {
      return `입력값을 확인해 주세요. ${messages.join(" ")}`;
    }
  }

  return objectMessage(detail) ?? objectMessage(payload) ?? fallback;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    request.addEventListener("load", () => {
      ...
      if (request.status < 200 || request.status >= 300) {
        const detail = apiErrorMessage(payload, request.status);
