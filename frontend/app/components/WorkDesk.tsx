...
export function WorkDesk({
  ...
  const workItemListPath = reviewQueueOnly
    ? "/api/work-items?review_queue_only=true"
    : "/api/work-items";
  ...
  const reloadWorkItems = useCallback(
    async (preferredId = selectedIdRef.current) => {
      const nextItems = await apiFetch<WorkItem[]>(workItemListPath);
      setItems(nextItems);
      const preferred =
        nextItems.find(
          (item) =>
    ...
    [applyItem, clearSelectedItem, statusFilter, workItemListPath],
  );
  ...
  function replaceAttachment(updated: Attachment) {
    ...
    setOcrDrafts((current) => ({
      ...
      [updated.id]:
        ...
        updated.text_extraction?.suggested_text ??
  ...
  async function replaceResident(residentId: string) {
    ...
    try {
      ...
      replaceItem(updated);
      setStatusFilter(updated.status === "dismissed" ? "dismissed" : "review");
      setSuccess(
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void apiFetch<WorkItem[]>(workItemListPath)
        .then((nextItems) => {
          setItems(nextItems);
          setOcrDrafts((current) => {
            const next = { ...current };
    ...
    <div className="drawer-layer">
      ...
      <aside className="workdesk-drawer" aria-label="AI 업무 정리함">
        <header className="drawer-header">
          <div>
            <span className="eyebrow">기록 검토</span>
            <h2>기록으로 정리할 대화</h2>
          </div>
        ...
        <div className="workdesk-note">
          채팅에서 돌봄기록으로 활용할 내용을 원문과 함께 확인합니다. 수정하거나
          사용하지 않을 수 있으며, 직원이 승인하기 전에는 공식 기록이 되지 않습니다.
        </div>
        <ResidentConfirmationInbox onChanged={() => { void reloadWorkItems(selected?.id); }} />
                    ...
                    {previewImage ? (
                      <div className="work-item-thumbnail">
                        <AttachmentDisplay
                          ...
                          compact
                          showCompactConfirmedText={false}
          ...
          {selected ? (
            ...
            >
              ...
              <section className="work-original">
                ...
                <ResidentLinkReview message={selected.message} canEdit={!selected.confirmed_at}
                  onChanged={() => void reloadWorkItems(selected.id)} />
                ...
                {selected.message.attachments.length > 0 ? (
                  <div className="work-attachments">
                    {selected.message.attachments.map((attachment) => (
                      <div
                        className={`work-attachment-item ${
                          attachment.mime_type.startsWith("image/")
                            ? "work-attachment-image-review"
                            : ""
                        }`}
                        key={attachment.id}
                      >
                          ...
                          <section className="ocr-review-card">
                              ...
                              <span>
                                {attachment.text_extraction
                                  ? {
                                      ...
                                      reviewed: "담당자 확인 완료",
                                      no_text: "글자를 찾지 못함",
                                      not_required: "판독 필요 없음",
                                    }[attachment.text_extraction.status]
                                  : "아직 실행하지 않음"}
                              </span>
                            </div>
                            {attachment.text_extraction?.status === "failed" && attachment.text_extraction.error_message ? (
                                        ...
                                        <small>
                                          {candidate.source === "confirmed_history"
                                            ? candidate.support_count >= 2
                                              ? `반복 확인 ${candidate.support_count}건`
                                              : "과거 확인 1건"
                                            : candidate.source ===
                            ...
                            <details className="ocr-more-actions">
                              ...
                              <div className="ocr-actions">
                                {attachment.mime_type.startsWith("image/") ? <button type="button" className="resident-choice-button"
                                  disabled={ocrBusyId === attachment.id} onClick={() => {
                                    setOcrBusyId(attachment.id);
                                    void apiFetch(`/api/attachments/${attachment.id}/photo-reading`, { method: "PATCH", body: JSON.stringify({ decision: "not_required" }) })
                                      .then(() => reloadWorkItems(selected.id)).catch(() => setError("사진 판독 선택을 저장하지 못했습니다."))
                                      .finally(() => setOcrBusyId(""));
                                  }}>판독 필요 없음</button> : null}
                                {attachment.mime_type.startsWith("image/") &&
                                attachment.text_extraction ? (
                                  <>
                                    <button
