...
기본 Compose는 Web Push를 자동으로 활성화하지 않습니다. 별도 설정이 필요한 경우 [알림 복구 및 제한 중계 안내](WEB_PUSH_RECOVERY.md)를 참고하세요. 운영 비밀키나 기기 구독을 복사하지 않고, 설치자가 만든 키와 해당 기기의 명시적 동의로 설정합니다. 선택적 중계 예시는 기본 설치나 전체 인터넷 차단을 대신하는 구성이 아닙니다.
