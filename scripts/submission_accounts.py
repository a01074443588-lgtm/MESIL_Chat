...
def load_submission_accounts(
    ...
    for raw_line in text.splitlines():
        ...
        if line.startswith("[") and line.endswith("]"):
            normalized_role = line.lower().replace(" ", "")
            if "보고 작성자" in line or normalized_role == "[role:care_a]":
                current_role = "care_a"
            elif "실시간 수신자" in line or normalized_role == "[role:care_b]":
                current_role = "care_b"
            elif "업무함 검토자" in line or normalized_role == "[role:social]":
                current_role = "social"
            else:
                current_role = None
            continue
        ...
        if current_role and (
            line.startswith("아이디:") or line.lower().startswith("username:")
        ):
...
def load_submission_password(path: Path = DEFAULT_CREDENTIALS_PATH) -> str:
    text = path.read_text(encoding="utf-8")
    passwords = set(
        re.findall(r"(?im)^(?:비밀번호|password):\s*(\S+)$", text)
    )
    if len(passwords) != 1:
        raise RuntimeError(
