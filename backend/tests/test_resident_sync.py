...
def test_finalist_coded_dev_blocks_carefor_roster_and_saved_preview(monkeypatch):
    block_detail = (
    ...
    with TestClient(app) as client:
        ...
        with SessionLocal() as db:
            ...
            batch = ResidentSyncBatch(
                ...
                original_name="carefor_facility_residents.local.json",
                file_sha256="0" * 64,
                status="preview",
                summary={"new": 1},
            ...
            item = ResidentSyncItem(
                batch_id=batch.id,
                organization_id=admin.organization_id,
                external_id="carefor:facility:resident:finalist-guard",
                change_type="new",
                status="pending",
                incoming_payload={
                    "display_name": "어르9999",
        ...
        monkeypatch.setattr(settings, "database_schema", "smcodi_finalist")
        ...
        assert status_response.json()["detail"] == block_detail
        ...
        assert preview_response.json()["detail"] == block_detail
        ...
        assert batch_id not in {row["id"] for row in batch_list.json()}

        detail_response = client.get(
            f"/api/admin/resident-sync/batches/{batch_id}",
            headers=ORIGIN,
        )
        assert detail_response.status_code == 409
        assert detail_response.json()["detail"] == block_detail

        apply_response = apply_items(client, batch_id, [item_id])
        assert apply_response.status_code == 409
        assert apply_response.json()["detail"] == block_detail
