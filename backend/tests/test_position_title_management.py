...
def test_admin_manages_position_titles_and_employee_selection() -> None:
    with TestClient(app) as client:
        ...
        assert not any(item["name"] == "원장" for item in initial.json())
        ...
        removed_from_selection = client.patch(
            f"/api/position-titles/{position_id}",
            json={"is_active": False},
            headers=ORIGIN,
        )
        assert removed_from_selection.status_code == 200, removed_from_selection.text
        assert removed_from_selection.json()["is_active"] is False
        employee_after_removal = next(
            item for item in client.get("/api/employees").json()
            if item["id"] == employee_id
        )
        assert employee_after_removal["position_title"] == "부원장"
