...
def test_external_summary_context_hides_only_resident_and_direct_identifiers():
    entries = [
        {
            "resident": "가나다",
            "body": (
                "가나다 어르신이 점심을 절반 드셨고 보호자 전화 010-1234-5678, "
                "주민번호 450101-1234567, 계좌 123-456-789012를 확인했습니다."
            ),
            "attachment_text": "송영 후 혈압 120/80을 확인했습니다.",
        }
    ]

    safe_entries, aliases = local_ai.deidentify_external_room_entries(
        entries,
        resident_names=["가나다"],
    )
    serialized = str(safe_entries)

    assert "가나다" not in serialized
    assert "010-1234-5678" not in serialized
    assert "450101-1234567" not in serialized
    assert "123-456-789012" not in serialized
    assert "점심을 절반" in serialized
    assert "혈압 120/80" in serialized
    assert local_ai.restore_external_resident_names("RESIDENT_001 확인", aliases) == (
        "가나다 확인"
    )


def test_external_summary_context_serializes_uuid_metadata():
    resident_id = uuid4()
    safe_entries, _ = local_ai.deidentify_external_room_entries(
        [{"resident_refs": [{"id": resident_id, "name": "sample resident"}]}],
        resident_names=["sample resident"],
    )

    assert safe_entries[0]["resident_refs"][0]["id"] == resident_id
    assert safe_entries[0]["resident_refs"][0]["name"] == "RESIDENT_001"


def test_numbered_room_entries_serializes_nested_uuid_metadata_without_mutation():
    attachment_id = uuid4()
    comment_id = uuid4()
    evidence_id = uuid4()
    entries = [
        {
            "number": 1,
            "attachment": {"metadata": {"id": attachment_id}},
            "comments": [{"id": comment_id, "evidence": {"id": evidence_id}}],
        }
    ]

    rendered = local_ai._numbered_room_entries(entries)

    assert str(attachment_id) in rendered
    assert str(comment_id) in rendered
    assert str(evidence_id) in rendered
    assert entries[0]["attachment"]["metadata"]["id"] == attachment_id
    assert entries[0]["comments"][0]["id"] == comment_id
    assert entries[0]["comments"][0]["evidence"]["id"] == evidence_id


def test_structured_search_summary_uses_attachment_text_instead_of_boilerplate():
    result = local_ai.structured_room_summary(
        [
            {
                "number": 1,
                "created_at": "2026-08-11T01:00:00+00:00",
                "resident": "가나다",
                "resident_names": ["가나다", "라마바"],
                "body": "파일을 첨부했습니다.",
                "attachment_text": "점심 식사 후 복약을 확인했습니다.",
                "comments": [],
    ...
    assert "[날짜·시간대별]" in result.summary
    assert "[어르신별 변화]" in result.summary
    assert "[주의점]" in result.summary
    assert "[미해결 업무]" in result.summary
    assert "점심 식사 후 복약" in result.summary
    assert "파일을 첨부했습니다" not in result.summary
    assert "가나다" in result.summary
    assert "라마바" in result.summary


def test_explicit_nemotron_selection_uses_deidentified_external_route(monkeypatch):
    monkeypatch.setattr(local_ai.settings, "ai_review_external_enabled", False)
    monkeypatch.setattr(
        local_ai,
        "_room_summary_with_nvidia",
        lambda *args, **kwargs: (
            '{"summary":"검색 결과의 주요 업무 내용과 확인할 사항을 근거 [1]에서 '
            '확인해 정리했습니다.","references":[1]}'
        ),
    )

    result = local_ai.summarize_room_messages(
        entries=[{"number": 1, "body": "시험 내용"}],
...
def test_nemotron_connection_health_uses_runtime_file_and_synthetic_context(
    monkeypatch,
    tmp_path,
):
    key_file = tmp_path / "nvidia_api_key.local"
    key_file.write_text("synthetic-file-key", encoding="utf-8")
    monkeypatch.setattr(local_ai.settings, "nvidia_api_key_file", str(key_file))
    monkeypatch.setattr(
        local_ai.settings,
        "nvidia_api_key",
        SecretStr("environment-key-must-not-win"),
    )
            ...
            "choices": [
                {
                    "message": {
                        "content": json.dumps(
                            {
                                "summary": (
                                    "합성 자료만 사용한 Nemotron 연결 점검이 정상적으로 "
                                    "완료되었으며 실제 기관 기록은 포함하지 않았습니다. [1]"
                                ),
                                "references": [1],
                            },
                            ensure_ascii=False,
    ...
    monkeypatch.setattr(local_ai, "_request_json", mock_request)

    assert (
        local_ai.verify_nemotron_connection(timeout_seconds=7)
        == local_ai.settings.nvidia_nemotron_model
    )
    ...
    assert "실제 어르신이나 기관 업무 기록을 포함하지 않습니다" in serialized_payload
    assert "environment-key-must-not-win" not in serialized_payload


def test_nemotron_connection_falls_back_to_super_and_returns_actual_model(
    monkeypatch,
):
    calls: list[tuple[str, bool]] = []
    primary_model = "nvidia/nemotron-3-ultra-550b-a55b"
    monkeypatch.setattr(local_ai.settings, "nvidia_nemotron_model", primary_model)

    def fake_summary(*args, **kwargs):
        del args
        calls.append((kwargs["model"], kwargs["key_file_only"]))
        if kwargs["model"] == primary_model:
            raise local_ai.AiModelUnavailableError("model unavailable")
        return "합성 연결 확인 결과입니다. [1]"

    monkeypatch.setattr(local_ai, "_room_summary_with_nvidia", fake_summary)

    actual_model = local_ai.verify_nemotron_connection(timeout_seconds=7)

    assert actual_model == local_ai.NVIDIA_NEMOTRON_SUPER_MODEL
    assert calls == [
        (primary_model, True),
        (local_ai.NVIDIA_NEMOTRON_SUPER_MODEL, True),
    ]


def test_search_summary_falls_back_once_and_records_actual_model(monkeypatch):
    calls: list[str] = []
    primary_model = "nvidia/nemotron-3-ultra-550b-a55b"
    monkeypatch.setattr(local_ai.settings, "nvidia_nemotron_model", primary_model)
    ...
    def fake_summary(*args, **kwargs):
        ...
        if kwargs["model"] == primary_model:
            raise local_ai.AiInvalidResponseError("invalid response")
        return (
            '{"summary":"검색 결과의 주요 내용을 근거 [1]에서 확인했습니다.",'
            '"references":[1]}'
        )
    ...
    assert calls == [primary_model, local_ai.NVIDIA_NEMOTRON_SUPER_MODEL]
    assert result.provider == "nvidia"
    assert result.model == local_ai.NVIDIA_NEMOTRON_SUPER_MODEL


def test_search_summary_does_not_resend_on_non_compatibility_failure(monkeypatch):
    calls: list[str] = []
    primary_model = "nvidia/nemotron-3-ultra-550b-a55b"
    monkeypatch.setattr(local_ai.settings, "nvidia_nemotron_model", primary_model)
    monkeypatch.setattr(local_ai.settings, "ai_review_external_enabled", True)
    ...
    def fail_auth(*args, **kwargs):
        del args
        calls.append(kwargs["model"])
        raise local_ai.LocalAiError("AI 서버 오류(401).")

    monkeypatch.setattr(local_ai, "_room_summary_with_nvidia", fail_auth)

    with pytest.raises(local_ai.LocalAiError, match="401"):
        local_ai.summarize_room_messages(
            entries=[{"number": 1, "body": "시험 내용"}],
            external_allowed=True,
            preferred_provider="nvidia",
        )
...
def test_draft_review_fallback_records_both_actual_model_attempts(monkeypatch):
    calls: list[str] = []
    primary_model = "nvidia/nemotron-3-ultra-550b-a55b"
    monkeypatch.setattr(local_ai.settings, "ai_review_provider", "nvidia")
    monkeypatch.setattr(local_ai.settings, "ai_review_external_enabled", True)
    monkeypatch.setattr(local_ai.settings, "nvidia_nemotron_model", primary_model)
    ...
    monkeypatch.setattr(local_ai, "_review_with_nvidia", review)
    ...
    assert calls == [primary_model, local_ai.NVIDIA_NEMOTRON_SUPER_MODEL]
    assert result.model == local_ai.NVIDIA_NEMOTRON_SUPER_MODEL
    assert [attempt["model"] for attempt in result.attempts] == calls
...
def test_nemotron_room_summary_disables_thinking(monkeypatch, tmp_path):
    ...
    assert response_format["json_schema"]["name"] == "room_summary"
    schema = response_format["json_schema"]["schema"]
    assert schema["required"] == ["summary", "references"]
    assert schema["properties"]["references"]["items"]["maximum"] == 1
...
def test_nemotron_invalid_success_response_is_fallback_eligible(monkeypatch, tmp_path):
    key_file = tmp_path / "nvidia.key"
    key_file.write_text("test-key", encoding="utf-8")
    monkeypatch.setattr(local_ai.settings, "nvidia_api_key", None)
    monkeypatch.setattr(local_ai.settings, "nvidia_api_key_file", str(key_file))
    monkeypatch.setattr(local_ai, "_request_json", lambda **kwargs: {"choices": []})

    with pytest.raises(local_ai.AiInvalidResponseError):
        local_ai._room_summary_with_nvidia(
            [{"number": 1, "body": "시험 내용"}],
...
def test_nemotron_work_review_uses_json_mode_and_restores_resident_alias(
    ...
    response_format = captured["response_format"]
    assert response_format["type"] == "json_schema"
    assert response_format["json_schema"]["name"] == "record_draft"
    schema = response_format["json_schema"]["schema"]
...
def test_work_review_serializes_nested_uuid_metadata(
    monkeypatch,
    tmp_path,
    provider,
):
    ...
    def request_json(**kwargs):
        captured.update(
            json.loads(kwargs["payload"]["messages"][1]["content"])
        )
        response_draft = {
            **VALID_DRAFT,
            "corrected_text": VALID_DRAFT["corrected_text"].replace(
        ...
        serialized_draft = json.dumps(response_draft, ensure_ascii=False)
        if provider == "nvidia":
            return {"choices": [{"message": {"content": serialized_draft}}]}
        return {"message": {"content": serialized_draft}}
    ...
    if provider == "nvidia":
        ...
        monkeypatch.setattr(local_ai.settings, "nvidia_api_key", None)
        monkeypatch.setattr(local_ai.settings, "nvidia_api_key_file", str(key_file))
        local_ai._review_with_nvidia(
            model="nvidia/test-ultra",
            user_payload=user_payload,
        )
    else:
        local_ai._review_with_ollama(
            model="ollama/test-model",
            user_payload=user_payload,
        )

    source = captured["source"]
    assert source["attachment"]["metadata"]["id"] == str(attachment_id)
    assert source["comments"][0]["id"] == str(comment_id)
    assert source["comments"][0]["evidence"]["id"] == str(evidence_id)
    assert user_payload["source"]["attachment"]["metadata"]["id"] == attachment_id
    assert user_payload["source"]["comments"][0]["id"] == comment_id
    assert user_payload["source"]["comments"][0]["evidence"]["id"] == evidence_id
...
def test_request_json_marks_404_without_exposing_response_body(monkeypatch):
    response_marker = "provider-response-must-not-be-logged"

    def fake_urlopen(request, timeout):
        del request, timeout
        raise local_ai.HTTPError(
            "https://example.invalid/v1/chat/completions",
            404,
            "not found",
            {},
            BytesIO(response_marker.encode("utf-8")),
        )

    monkeypatch.setattr(local_ai, "urlopen", fake_urlopen)

    with pytest.raises(local_ai.AiModelUnavailableError) as caught:
        local_ai._request_json(
            url="https://example.invalid/v1/chat/completions",
...
def test_selected_local_summary_uses_configured_model_fallbacks(monkeypatch):
    ...
    def local_summary(*args, **kwargs):
        del args
        calls.append((kwargs["model"], kwargs["timeout_seconds"]))
        if kwargs["model"] == "qwen3.6:35b":
            raise local_ai.LocalAiError("첫 모델 응답 실패")
        return "주요 내용: 식사량을 확인했습니다. [1]"
    ...
    assert calls == [("qwen3.6:35b", 45), ("gemma4:e4b", 45)]
    assert result.provider == "ollama"
    assert result.model == "gemma4:e4b"


def test_record_summary_accepts_attachment_comment_and_rendered_created_at():
    entries = [
        {
            "number": 1,
            "resident": "시설(가명)003",
            ...
            "body": "파일을 첨부했습니다.",
            "attachment_text": "혈압 120/80을 확인했습니다.",
            "comments": ["간호팀에 전달했습니다."],
        }
    ]
    summary = (
        "[한눈에 보기]\n8월 12일 혈압 120/80 확인 기록입니다. [1]\n"
        "[먼저 확인]\n- 혈압 경과를 확인합니다. [1]\n"
        "[이미 한 일]\n- 간호팀에 전달했습니다. [1]\n"
        "[다음 업무 제안]\n- 원문을 확인합니다. [1]"
    )
