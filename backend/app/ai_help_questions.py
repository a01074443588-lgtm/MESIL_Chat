...
_RESIDENT_CODE = re.compile(r"어르\s*\d{4}")
...
def classify_ai_help_question(
    ...
    *,
    ...
    if re.search(r"우리\s*(?:기관|시설)|기관\s*전체|전체\s*어르신|어르신들", normalized) and re.search(
        r"돌봄|특성|특징|잘하|잘\s*하|최근|상태|모습|반응|행동|기록", normalized
    ):
    ...
    has_specific_resident_care_fact_request = bool(
        has_specific_resident
        and _SPECIFIC_RESIDENT_CARE_FACT.search(normalized)
        and _SPECIFIC_RESIDENT_CARE_FACT_ACTION.search(normalized)
    )
    if (
        has_scoped_lookup
        or has_specific_resident_record_request
        or has_specific_resident_care_fact_request
