...
class AiModelUnavailableError(LocalAiError):
    """The requested provider model is unavailable at the configured endpoint."""


class AiInvalidResponseError(LocalAiError):
    """The provider returned HTTP success without a locally usable result."""
...
def _nvidia_model_candidates() -> tuple[str, ...]:
    """Return the configured model first and the bounded Super fallback second."""

    models = (
        settings.nvidia_nemotron_model.strip(),
        NVIDIA_NEMOTRON_SUPER_MODEL,
    )
    return tuple(dict.fromkeys(model for model in models if model))


def _nvidia_fallback_allowed(exc: LocalAiError) -> bool:
    """Allow one cross-model retry only for model or response compatibility."""

    return isinstance(exc, (AiModelUnavailableError, AiInvalidResponseError))
...
def _record_summary_source_text(entries: list[dict[str, Any]]) -> str:
    ...
    for entry in entries:
        # The providers receive every source field except the internal evidence
        # number (see _numbered_room_entries).  Validate against that same source
        # envelope so reviewed attachment text and comments are not mistaken for
        # invented facts.
        details = {key: value for key, value in entry.items() if key != "number"}
        source_parts.append(json.dumps(details, ensure_ascii=False, default=str))

        # A model may render an ISO timestamp as a Korean date/time.  Add only
        # deterministic representations of the timestamp that was actually sent;
        # this keeps the numeric guard strict for unrelated values.
        raw_created_at = entry.get("created_at")
        try:
            created_at = (
                raw_created_at
                if isinstance(raw_created_at, datetime)
                else datetime.fromisoformat(str(raw_created_at or ""))
            )
            if created_at.tzinfo is None:
                created_at = created_at.replace(tzinfo=timezone.utc)
        except (TypeError, ValueError):
            continue
        for timestamp in (created_at, created_at.astimezone(kst)):
            meridiem = "오전" if timestamp.hour < 12 else "오후"
            twelve_hour = timestamp.hour % 12 or 12
            source_parts.append(
                f"{timestamp.year}년 {timestamp.month}월 {timestamp.day}일 "
                f"{timestamp.hour}시 {timestamp.minute}분 "
                f"{meridiem} {twelve_hour}시 {timestamp.minute}분"
            )
    return "\n".join(source_parts)
...
def _numbered_room_entries(entries: list[dict[str, Any]]) -> str:
    ...
    for index, entry in enumerate(entries, 1):
        ...
        details = {key: value for key, value in entry.items() if key != "number"}
        lines.append(
            f"[{number}] {json.dumps(details, ensure_ascii=False, default=str)}"
        )
    return "\n".join(lines)
...
_PHONE_PATTERN = re.compile(
    r"(?<!\d)(?:01[016789]-?\d{3,4}-?\d{4}|0(?:2|[3-6][1-5])-?\d{3,4}-?\d{4})(?!\d)"
)
_RESIDENT_NUMBER_PATTERN = re.compile(r"(?<!\d)\d{6}-?[1-4]\d{6}(?!\d)")
_ACCOUNT_PATTERN = re.compile(
    r"((?:계좌|통장|은행)(?:번호)?\s*[:：]?\s*)(\d[\d-]{8,}\d)",
    flags=re.IGNORECASE,
)


def _redact_direct_identifiers(value: Any) -> Any:
    if isinstance(value, str):
        redacted = _PHONE_PATTERN.sub("[전화번호 가림]", value)
        redacted = _RESIDENT_NUMBER_PATTERN.sub("[주민번호 가림]", redacted)
        return _ACCOUNT_PATTERN.sub(r"\1[계좌번호 가림]", redacted)
    if isinstance(value, list):
        return [_redact_direct_identifiers(item) for item in value]
    if isinstance(value, dict):
        return {
            key: _redact_direct_identifiers(item)
            for key, item in value.items()
        }
    return value


def deidentify_external_room_entries(
    entries: list[dict[str, Any]],
    *,
    resident_names: list[str] | None = None,
) -> tuple[list[dict[str, Any]], dict[str, str]]:
    """Hide resident names/direct identifiers without shortening the context."""

    serialized = json.dumps(entries, ensure_ascii=False, default=str)
    names = set(_RESIDENT_ALIAS_PATTERN.findall(serialized))
    names.update(
        name.strip()
    ...
    original_to_alias = {
        name: f"RESIDENT_{index:03d}"
        for index, name in enumerate(sorted(names, key=lambda item: (-len(item), item)), 1)
    }
    aliased = _replace_nested_text(entries, original_to_alias)
    redacted = _redact_direct_identifiers(aliased)
    return redacted, original_to_alias


def restore_external_resident_names(
    value: str,
    original_to_alias: dict[str, str],
) -> str:
    return _restore_resident_names(value, original_to_alias)


def _entry_summary_text(entry: dict[str, Any]) -> str:
    body = str(entry.get("body") or "").strip()
    attachment_text = str(entry.get("attachment_text") or "").strip()
    generic_attachment_bodies = {
        "파일을 첨부했습니다.",
        "사진을 첨부했습니다.",
        "음성을 첨부했습니다.",
    }
    parts: list[str] = []
    if body and (body not in generic_attachment_bodies or not attachment_text):
        parts.append(body)
    if attachment_text:
...
def _summary_excerpt(value: str, limit: int = 116) -> str:
...
def structured_room_summary(entries: list[dict[str, Any]]) -> RoomSummaryResult:
    """Fast evidence-linked fallback that never copies only attachment boilerplate."""

    if not entries:
        raise LocalAiError("요약할 검색 결과가 없습니다.")
    kst = timezone(timedelta(hours=9))
    time_groups: dict[str, list[str]] = defaultdict(list)
    resident_groups: dict[str, list[str]] = defaultdict(list)
    risk_lines: list[str] = []
    ...
    theme_terms = {
        ...
        "안전": ("낙상", "넘어", "부딪", "안전바"),
        "송영·이동": ("송영", "차량", "귀가", "모시"),
        "식사·수분": ("식사", "아침", "점심", "저녁", "수분"),
    }
    risk_terms = ("낙상", "상처", "출혈", "통증", "발진", "설사", "구토", "응급", "복약")
    theme_counts: Counter[str] = Counter()

    for position, entry in enumerate(entries, 1):
        number = entry.get("number") if isinstance(entry.get("number"), int) else position
        ...
        try:
            ...
            if created_at.tzinfo is None:
                created_at = created_at.replace(tzinfo=timezone.utc)
            local_at = created_at.astimezone(kst)
            hour = local_at.hour
            time_label = "오전" if 5 <= hour < 12 else "오후" if hour < 18 else "저녁" if hour < 22 else "야간"
            date_label = f"{local_at.month}월 {local_at.day}일 {time_label}"
        except ValueError:
            date_label = "시간 확인 필요"
        line = f"- {excerpt} [{number}]"
        time_groups[date_label].append(line)
        residents = [
            str(name).strip()
            for name in (entry.get("resident_names") or [])
            if str(name).strip()
        ]
        resident = str(entry.get("resident") or "").strip()
        if resident:
            residents.insert(0, resident)
        residents = list(dict.fromkeys(residents))
        for resident_name in residents:
            resident_groups[resident_name].append(line)
        ...
        references.append(
            f"- [{number}] {date_label} · {', '.join(residents) or '일반 대화'}"
        )

    sections = ["[기간 요약]", f"검색 결과 {len(entries)}건을 날짜·시간대와 대상별로 정리했습니다."]
    sections.append("\n[날짜·시간대별]")
    for label, lines in list(time_groups.items())[:12]:
        sections.append(f"{label}\n" + "\n".join(lines[:3]))
    sections.append("\n[어르신별 변화]")
    if resident_groups:
        for resident, lines in list(resident_groups.items())[:12]:
            sections.append(f"{resident}\n" + "\n".join(lines[:3]))
    else:
        sections.append("- 특정 어르신이 연결된 내용이 없습니다.")
    sections.append("\n[반복 상황]")
    repeated = [(theme, count) for theme, count in theme_counts.most_common() if count >= 2]
    sections.extend(
        [f"- {theme}: {count}건" for theme, count in repeated[:8]]
        or ["- 같은 주제로 반복된 내용이 확인되지 않았습니다."]
    )
    sections.append("\n[주의점]")
    sections.extend(risk_lines[:8] or ["- 별도 주의 키워드가 확인되지 않았습니다."])
    sections.append("\n[미해결 업무]")
    sections.extend(unresolved_lines[:8] or ["- 검색 결과에 미완료로 표시된 업무가 없습니다."])
    sections.append("\n[근거 메시지]")
    sections.extend(references[:30])
    return RoomSummaryResult(
        summary="\n".join(sections),
        provider="rules",
        model="structured-search-summary-v2",
        elapsed_ms=0,
    )
...
def nemotron_is_configured() -> bool:
    ...
    return bool(api_key and api_key != "PASTE_NVIDIA_API_KEY_HERE")


def local_summary_provider_status() -> tuple[bool, str]:
    if settings.ai_review_provider.strip().lower() == "stub":
        return True, "시험용 로컬 AI가 준비됐습니다."
    request = Request(
        f"{settings.ai_review_base_url.rstrip('/')}/api/tags",
    ...
    try:
        ...
        return False, "로컬 LLM 서버에 연결할 수 없습니다."
    models = payload.get("models") if isinstance(payload, dict) else None
    if not isinstance(models, list) or not models:
        return False, "로컬 LLM 서버에 사용 가능한 모델이 없습니다."
    return True, f"로컬 LLM 모델 {len(models)}개를 사용할 수 있습니다."
...
def _nvidia_api_key(*, file_only: bool = False) -> str:
    api_key = (
        settings.nvidia_api_key.get_secret_value().strip()
        if not file_only and settings.nvidia_api_key is not None
        else ""
    )
    if not api_key:
...
def _room_summary_response_format(source_count: int) -> dict[str, Any]:
    # NVIDIA hosted NIM 백엔드 중 일부는 제약 조건이 없는 json_object를
    # StructuredOutputsParams로 변환하며 HTTP 400을 반환한다. 실제로 필요한
    # 두 필드를 JSON Schema로 명시하고, 응답 뒤의 기존 근거번호 검증도 유지한다.
    return {
        "type": "json_schema",
        "json_schema": {
            "name": "room_summary",
...
def _work_review_response_format() -> dict[str, Any]:
    schema = RecordDraft.model_json_schema()
...
def _room_summary_with_nvidia(
    ...
    *,
    ...
    key_file_only: bool = False,
) -> str:
    actual_model = model or settings.nvidia_nemotron_model
    api_key = _nvidia_api_key(file_only=key_file_only)
    if not api_key or api_key == "PASTE_NVIDIA_API_KEY_HERE":
        raise LocalAiError(
            "Nemotron 운영 등록 파일에서 사용할 설정을 확인하지 못했습니다."
        )
    aliased_entries, resident_aliases = deidentify_external_room_entries(
        entries,
    ...
    provider_timeout_seconds = timeout_seconds or (
        min(
    ...
    try:
        ...
        raise AiInvalidResponseError(
            "Nemotron 응답이 로컬 검증을 통과하지 못했습니다."
        ) from exc
    return summary


def _room_summary_with_nvidia_fallback(
    entries: list[dict[str, Any]],
    *,
    purpose: str | None = None,
    resident_names: list[str] | None = None,
    ...
    models = _nvidia_model_candidates()
    for index, candidate_model in enumerate(models):
        try:
            summary = _room_summary_with_nvidia(
                entries,
                model=candidate_model,
                purpose=purpose,
                resident_names=resident_names,
                timeout_seconds=timeout_seconds,
                key_file_only=key_file_only,
            )
        except LocalAiError as exc:
            has_fallback = index + 1 < len(models)
            if not has_fallback or not _nvidia_fallback_allowed(exc):
                raise
            logger.warning(
                "nemotron_model_fallback from_model=%s to_model=%s failure_kind=%s",
                candidate_model,
                models[index + 1],
                type(exc).__name__,
            )
    ...
    raise LocalAiError("사용 가능한 Nemotron 모델이 없습니다.")


def verify_nemotron_connection(*, timeout_seconds: float = 20) -> str:
    """Verify the runtime key file with synthetic, non-institutional context."""

    synthetic_name = "연결점검가명001"
    _, model = _room_summary_with_nvidia_fallback(
        [
            {
                "number": 1,
                ...
                "sender": "합성연결점검",
                "resident": synthetic_name,
                "resident_names": [synthetic_name],
                "body": (
                    "Nemotron 연결 확인만을 위한 합성 자료이며 실제 어르신이나 "
                    "기관 업무 기록을 포함하지 않습니다."
                ),
                "comments": [],
                "attachment_text": "",
                "source_label": "합성 연결 점검",
                "action_status": None,
            }
        ],
        resident_names=[synthetic_name],
...
def _room_summary_with_ollama(
    ...
    *,
    ...
    response = _request_json(
        ...
        timeout_seconds=timeout_seconds or settings.ai_review_timeout_seconds,
    )
    try:
...
def summarize_room_messages(
    *,
    ...
    resident_names: list[str] | None = None,
    timeout_seconds: float | None = None,
    central_feature: str | None = None,
) -> RoomSummaryResult:
    if central_feature is not None:
        central_result = _central_workdesk_summary(
            entries=entries, feature=central_feature, purpose=purpose,
            all_synthetic=external_allowed, timeout_seconds=timeout_seconds,
        )
        if central_result is not None:
            return central_result
    provider = (preferred_provider or settings.ai_review_provider).strip().lower()
    if provider == "rules":
        return structured_room_summary(entries)
    if provider == "stub":
        lines = [
    ...
    if provider in {"chain", "nvidia", "nemotron"}:
        explicitly_selected_external = preferred_provider in {"nvidia", "nemotron"}
        if external_allowed and (
            settings.ai_review_external_enabled or explicitly_selected_external
        ):
            started = perf_counter()
            try:
                nvidia_summary, actual_model = _room_summary_with_nvidia_fallback(
                    entries,
                    purpose=purpose,
                    resident_names=resident_names,
                    timeout_seconds=timeout_seconds,
            ...
            raise LocalAiError("외부 AI 요약 선택 또는 전송 허용 설정이 필요합니다.")

    if provider in {"chain", "ollama", "local"}:
        models = _model_list(settings.ai_review_local_models)
        if not models:
            models = [settings.ai_review_model]
...
def _central_workdesk_summary(
    *, entries: list[dict[str, Any]], feature: str, purpose: str | None,
    all_synthetic: bool, timeout_seconds: float | None,
) -> RoomSummaryResult | None:
    """Warm-only internal LLM path for the two workdesk summary endpoints.

    No legacy cross-model fallback, cold load, external provider, or config write.
    The caller must complete content-free model preparation before retrying cold.
    """
    import asyncio
    import httpx
    from .ai_settings_store import load_ai_settings, effective_central_models, central_feature_selection
    from . import record_narrative
    from .record_text_ai import RecordModelError
    from .workdesk_briefing import (
        BriefingDraft, BriefingReview, DRAFT_INSTRUCTION, REVIEW_INSTRUCTION,
        validate_claims, review_items, render_verified,
    )

    if feature not in {"search_summary", "document_text"}:
        raise LocalAiError("local_model_unconfigured")
    document, _ = load_ai_settings()
    policy = effective_central_models(document)
    selected = central_feature_selection(policy, feature)
    if policy.default_provider == "rules" and selected is None:
        return structured_room_summary(entries)
    # Preserve deliberately configured non-AI operation when no central model
    # exists, without letting stale legacy model settings override a selection.
    if selected is None and settings.ai_review_provider in {"stub", "rules"}:
        return None
    if selected is None or selected.provider != "ollama" or not selected.model:
        raise LocalAiError("local_model_unconfigured")
    if not all_synthetic and not policy.real_record_logging_verified:
        raise LocalAiError("logging_policy_unverified")
    base = policy.base_url or document.providers["ollama"].base_url or settings.ai_review_base_url
    entries = [{**entry, "number": index} for index, entry in enumerate(entries, 1)]
    text = _numbered_room_entries(entries)
    if not entries or len(text) > policy.max_input_chars:
        raise LocalAiError("context_limit")
    budget = min(policy.timeout_seconds, timeout_seconds) if timeout_seconds is not None else policy.timeout_seconds
    started = perf_counter()

    async def generate():
        async with asyncio.timeout(budget), httpx.AsyncClient(trust_env=False, follow_redirects=False) as client:
            info = await record_narrative.local_request(client, base, "/api/show", {"model": selected.model}, min(2, budget))
            if info.get("remote_model") or info.get("remote_host") or "cloud" in selected.model.lower():
                raise RecordModelError("remote_model_blocked")
            ready = await record_narrative.local_request(client, base, "/api/ps", None, min(2, budget))
            active = next((row for row in ready.get("models", []) if selected.model in (row.get("name"), row.get("model"))), None)
            if active is None or active.get("context_length") != policy.context_tokens:
                raise RecordModelError("model_cold")
            return await record_narrative.local_request(client, base, "/api/chat", {
                "model": selected.model, "stream": False, "think": False, "format": BriefingDraft.model_json_schema(),
                "keep_alive": record_narrative.model_retention(selected.model, policy.context_tokens),
                "options": {"temperature": 0, "num_ctx": policy.context_tokens, "num_predict": 1800},
                "messages": [{"role": "system", "content": DRAFT_INSTRUCTION},
                             {"role": "user", "content": text}],
            }, budget)
    try:
        response = asyncio.run(generate())
    except (TimeoutError, httpx.TimeoutException) as exc:
        raise LocalAiError("timeout") from exc
    except httpx.RequestError as exc:
        raise LocalAiError("model_connection_error") from exc
    except httpx.HTTPStatusError as exc:
        raise LocalAiError("model_server_error") from exc
    except RecordModelError as exc:
        raise LocalAiError(str(exc)) from exc
    if response.get("model") != selected.model or response.get("done") is not True:
        raise LocalAiError("model_response_invalid")
    if response.get("done_reason") in {"length", "max_tokens"}:
        raise LocalAiError("response_truncated")
    content = response.get("message", {}).get("content")
    if not isinstance(content, str):
        raise LocalAiError("model_response_invalid")
    try:
        claims = validate_claims(json.loads(content), entries,
            lambda text, sources: _validate_record_summary_faithfulness(text, sources, require_headings=False))
    except ValueError as exc:
        raise LocalAiError(str(exc) if not isinstance(exc, json.JSONDecodeError) else "response_format_invalid") from exc
    remaining = budget - (perf_counter() - started)
    if remaining <= 0:
        raise LocalAiError("timeout")

    async def review():
        async with asyncio.timeout(remaining), httpx.AsyncClient(trust_env=False, follow_redirects=False) as client:
            return await record_narrative.local_request(client, base, "/api/chat", {
                "model": selected.model, "stream": False, "think": False,
                "keep_alive": record_narrative.model_retention(selected.model, policy.context_tokens),
                "format": BriefingReview.model_json_schema(),
                "options": {"temperature": 0, "num_ctx": policy.context_tokens, "num_predict": 256},
                "messages": [{"role": "system", "content": REVIEW_INSTRUCTION},
                    {"role": "user", "content": json.dumps({"items": review_items(claims, entries)}, ensure_ascii=False)}],
            }, remaining)
    try:
        checked = asyncio.run(review())
    except (TimeoutError, httpx.TimeoutException) as exc:
        raise LocalAiError("timeout") from exc
    except (httpx.RequestError, httpx.HTTPStatusError, RecordModelError) as exc:
        raise LocalAiError("summary_review_unavailable") from exc
    if checked.get("model") != selected.model or checked.get("done") is not True:
        raise LocalAiError("model_response_invalid")
    try:
        verdict = json.loads(checked["message"]["content"])
    except (ValueError, KeyError, TypeError) as exc:
        raise LocalAiError("review_format_invalid") from exc
    try:
        summary = render_verified(claims, verdict, entries, purpose)
    except ValueError as exc:
        raise LocalAiError(str(exc)) from exc
    return RoomSummaryResult(summary=summary, provider="ollama", model=selected.model,
        ...
        try:
            ...
            if status_code == 404:
                raise AiModelUnavailableError(
                    "요청한 AI 모델을 이 API에서 사용할 수 없습니다(404)."
                ) from exc
            raise LocalAiError(f"AI 서버 오류({status_code}).") from exc
    ...
    *,
    ...
    response = _request_json(
        ...
        payload={
            ...
            "max_tokens": 3600,
            "chat_template_kwargs": {"enable_thinking": False},
            "response_format": _work_review_response_format(),
            "stream": False,
        },
