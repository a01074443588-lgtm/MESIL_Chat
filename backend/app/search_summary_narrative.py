...
정상적으로 근거가 충분하면 정확히 3개의 짧은 문장을 쓰되, 기록이 적으면 근거 없는 문장을 늘리지 마세요. 각 sentence는 70자 이내로 작성하고, 건수 통계나 날짜순 원문 목록으로 끝내지 마세요.
        ...
        async with asyncio.timeout(budget), httpx.AsyncClient(trust_env=False, follow_redirects=False) as client:
            ...
            for attempt in range(2):
                ...
                draft_response = await chat(
                    ...
                    + ("\n이전 결과에서 검증 실패가 있었습니다. 검증 가능한 문장만 다시 작성하세요." if attempt else ""),
                    instruction,
                    wire_schema,
                    300,
                )
