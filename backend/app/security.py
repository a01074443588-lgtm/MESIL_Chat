...
MENTOR_FULL_REVIEW_EXPERIENCE = "mentor_full"
REVIEWER_EXPERIENCES = {
    "care",
    "social_worker",
    "realtime_secondary",
...
def is_mentor_full_reviewer(user: User) -> bool:
    return (
        getattr(user, "_reviewer_experience", None)
        == MENTOR_FULL_REVIEW_EXPERIENCE
    )


def mentor_visible_resident_code(internal_code: str) -> bool:
    """Keep mentor review data on explicitly synthetic DEV fixtures only."""

    normalized = internal_code.strip().upper()
...
def validate_reviewer_session_user(
    ...
    *,
    ...
    if context is None:
        return None
    expected_username = settings.reviewer_username_for_experience(context.experience)
    expected_processor_access = context.experience in {
        "social_worker",
        MENTOR_FULL_REVIEW_EXPERIENCE,
    }
    if (
